...
impl<'a> HtmlParser<'a> {
  ...
  pub fn parse_nodes(&mut self) -> Vec<Node> {
    ...
    // we will iterate through our chars while checking if
    // there is some characters or not 'is_some()' true | false
...
fn parse_text_node(&mut self) -> Node {
  ...
  while self.chars.peek().map_or(false, |c| *c != '<') {
    ...
    let text_part = self.consumen_while(|x| !x.is_whitespace() && x != '<');
